...
<div class="card mb-4">
    <div class="card-body">
        <div class="row align-items-end g-3">
            <div class="col-md-6">
                <label class="form-label" for="formato_impresion">Formato de impresión</label>
                <select id="formato_impresion" class="form-select">
                    <?php foreach ($formatos as $valor => $label): ?>
                        <option value="<?php echo htmlspecialchars($valor); ?>" <?php echo $valor === $formatoSeleccionado ? 'selected' : ''; ?>>
            ...
            <div class="col-md-6 text-muted small">
                El formato seleccionado se aplica a las impresiones por empresa o funcionario.
            </div>
        </div>
    </div>
</div>
    ...
    <div class="col-lg-6">
        <div class="card h-100">
            <div class="card-body">
                ...
                <p class="text-muted">Genera la impresión de todos los aguinaldos de una empresa para un año.</p>
                <form method="get" action="<?php echo $baseUrl; ?>/index.php" target="_blank" class="row g-3">
                    <input type="hidden" name="route" value="aguinaldos/print-company">
    ...
    <div class="col-lg-6">
        <div class="card h-100">
            <div class="card-body">
                ...
                <p class="text-muted">Seleccione la empresa y luego el funcionario para imprimir su aguinaldo.</p>
                <form method="get" action="<?php echo $baseUrl; ?>/index.php" target="_blank" class="row g-3">
                    <input type="hidden" name="route" value="aguinaldos/print-individual">
                    <input type="hidden" name="formato" class="formato-input" value="<?php echo htmlspecialchars($formatoSeleccionado); ?>">
                    <div class="col-12">
                        <label class="form-label">Buscar empresa</label>
                        <input
...
const formatoInput = document.getElementById('formato_impresion');
...
const formatoHiddenInputs = document.querySelectorAll('.formato-input');
...
formatoInput?.addEventListener('change', () => {
    formatoHiddenInputs.forEach(input => {
        input.value = formatoInput.value;
    });
});
